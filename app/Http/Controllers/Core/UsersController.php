...
class UsersController extends Controller {
	...
	function postSave( Request $request, $id =0)
	{
		...
		if ($validator->passes()) {
			...
			if($request->id==''){
				$to = Input::get('email');
				$subject = 'Invitation to Clinic Management';
				$message = view('emails.invite',compact('data','pwd'));
			...
			if(!is_null($request->input('groups')))
			{
				...
				for($i=0; $i<count($groups); $i++)
				{
					...
					foreach($users as $row)
					{
						...
						Mail::raw($message,function ($m) use ($subject,$to)  {
							$m->to($to)->subject("[ " .CNF_APPNAME." ] ".$subject);
						});

						$notification= array(
							'userid' => $row->id,
							'title' => "New Mail from ". \Auth::user()->email,
							'note' => 	$request->input('subject'),
							'icon' => 'fa fa-envelope',
							'url' => '/mailbox',
							'entry_by' =>  $row->id
						);
						...
						$message = new Mailbox();
						$message->SenderID = \Session::get('uid');
						$message->ReceiverID = $row->id;
						$message->Subject = $request->input('subject');
						$message->Message = $request->input('message');
						$message->SentDate = date('Y-m-d H:i:s');
						$message->CreatedDate = date('Y-m-d H:i:s');
						$message->Status = 'inbox';
						$message->Category = 'System Message';
						$message->save();
