...
//								}
								foreach($_FILES[$field]['name'] as $key => $file ){
									...
									if($file!="")
									{
										$extension =pathinfo($file, PATHINFO_EXTENSION); //if you need extension of the file
										$rand = rand(1000,100000000);
										$file_name = strtotime(date('Y-m-d H:i:s')).'-'.$rand.'.'.$extension;
										if($file_name !='')
										{
											move_uploaded_file($_FILES[$field]['tmp_name'][$key],$destinationPath.'/'.$file_name);
											$files .= $file_name.',';
	...
	function postFilter( Request $request,$other = null)
	{
		$module = $this->module;
