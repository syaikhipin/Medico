...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static  function bulkForm( $field, $forms = array(), $value ='')
	{
		...
		switch($type)
		{
			...
			case 'text_datetime';
				//$form = "<input  type='text' name='$field{$bulk}'  class='form-control input-sm bootstrap-timepicker'  $mandatory value='{$value}'/> ";
				$form = "<input  type='time' name='$field{$bulk}'  placeholder='23:00' class='form-control input-sm '  $mandatory value='{$value}'/> ";
				break;
			...
			case 'select';
				...
				{
					$data= DB::table($option['lookup_table'])->get();
					//$data = DB::table($option['lookup_table'])->where('entry_by','=',\Session::get('uid'))->get();
					$opts = '';
					foreach($data as $row):
						$selected = '';
						if($value == $row->$option['lookup_key']) $selected ='selected="selected"';
	...
	public static function showUploadedFile($file,$path , $width = 50,$shape = 'circle',$height = 50) {

		$files =  public_path(). $path . $file ;

		if(substr( $file, 0, 4 ) =="http"){
			if (strpos($file, 'google') !== FALSE)
				return "<img src='".$file."&access_token=".\Session::get("gaccess_token")."' border='0' width='50' class='img-circle' /></a>";
			else
			return "<img src=".$file." border='0' width='50' class='img-circle' /></a>";
		}
		if(file_exists($files ) && $file !="") {
		//	echo $files ;
			$info = pathinfo($files);	
			if($info['extension'] == "jpg" || $info['extension'] == "jpeg" || $info['extension'] == "PNG" ||  $info['extension'] == "png" || $info['extension'] == "gif" || $info['extension'] == "JPG")
			{
				...
				if($shape == 'square')
				{
					return '<span style="padding: 4px"><a href="'.url( $path_file . $file).'" target="_blank" class="previewImage fancybox">
				<img src="'.asset( $path_file . $file ).'" border="0" width="'. $width .'"  height ="'.$height.'"class="img"  /></a></span>';
				}
				return '<p><a href="'.url( $path_file . $file).'" target="_blank" class="previewImage">
				<img src="'.asset( $path_file . $file ).'" border="0" width="'. $width .'" class="img-circle" height="'.$height.'" style="max-height:'.$width.'px" /></a></p>';
			} else {
				$path_file = str_replace("./","",$path);
			...
			if($shape == 'square' )
			{
				return "<img src='".asset('/uploads/images/no-image.png')."' border='0' width='".$width."' class='img' /></a>";
			}
	...
	public static function gridDisplayView($val , $field, $arr,$condition= null) {
		$arr = explode(':',$arr);
		...
		if(isset($arr['0']) && $arr['0'] ==1)
		{
			...
			if(!is_null($condition)){
				...
				$Q = DB::select(" SELECT ".str_replace("|",",",$arr['3'])." FROM ".$arr['1']." WHERE ".$arr['2']." = '".$val."' AND ". $condition);
				}
