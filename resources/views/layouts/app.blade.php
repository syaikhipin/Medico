...
<link rel="shortcut icon" href="{{ asset('sximo/images/'.CNF_FAVICON) }}" type="image/x-icon">
		...
		<link href="{{ asset('sximo/js/plugins/bootstrap.datetimepicker/css/bootstrap-datetimepicker.min.css')}}" rel="stylesheet">
			<link href="{{ asset('sximo/js/plugins/select2/new/select2.css')}}" rel="stylesheet">
...
{{--		<script type="text/javascript" src="{{ asset('sximo/js/plugins/select2/select2.min.js') }}"></script>--}}
		<script type="text/javascript" src="{{ asset('sximo/js/plugins/select2/new/select2.min.js') }}"></script>
	...
	<div id="wrapper">
		...
		<div class="footer fixed">
		    <div class="pull-right">
				@if(\Auth::user()->group_id==3 && \Auth::user()->payment_method=='0')
					<strong>Your Trial Period ends on <span class="text-danger"> {{ date('Y-M-d', strtotime('+'.CNF_TRIAL.' day',strtotime(\Auth::user()->created_at)))}}</span>. For continuation   <a href="{{ url('user/subscribe') }}">Subscribe</a> </strong>
				@endif
		    </div>
		    <div>
