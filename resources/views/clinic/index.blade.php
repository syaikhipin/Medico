			...
			<div class="sbox animated fadeInRight">
				...
				<div class="sbox-content">
					<div class="toolbar-line ">
						@if($access['is_add'] ==1)
							<a href="{{ URL::to('clinic/update') }}" class="tips btn btn-sm btn-white"  title="{{ Lang::get('core.btn_create') }}">
								...
								<tr>
										...
										@if($field['view'] =='1')
											...
											@if(SiteHelpers::filterColumn($limited ))
												...
												@if($field['field'] == 'isSponsored')
													<td width="100">
														@if($value==1)
															<label class="label label-info ">Sponsored</label>
														@else
															<a href="{{ URL::to( 'clinic/sponsor/'.$row->ClinicID) }}" class="label label-default" onclick="SximoModal(this.href,'Pay Sponsorship'); return false;" >Sponsor</a>
														@endif
													</td>
												@else
									...
									<td>
										@if($access['is_detail'] ==1)
											<a href="{{ URL::to('clinic/show/'.$row->ClinicID.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search "></i></a>
										@endif
										@if($access['is_edit'] ==1)
											@if(Auth::user()->group_id==1 && $row->entry_by=="" )
												<a  href="{{ URL::to('clinic/update/'.$row->ClinicID.'?return='.$return) }}" class="tips btn btn-xs btn-success" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit "></i></a>
